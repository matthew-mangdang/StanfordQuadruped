...
def main(use_imu=False):
    ...
    b_filter = butter_filter_live(cutoff_freq=10.0, dt=config.dt, order=1)  # Real-time Butterworth filter for acceleration, dt 0.01
    ...
    side_speed = min (0.0, config.max_y_velocity)
    ...
    while True:
        ...
        if a_filtered > 0.2:  # Threshold for detecting a hit (tune as needed)
    ...
    save_acceleration_plot(a_plot, filename="ax_trotting_raw.png")
    save_acceleration_plot(a_filtered_plot, filename="ax_trotting_filtered.png")
    np.save("acceleration_trotting_hit.npy", a_plot)
