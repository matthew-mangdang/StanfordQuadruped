...
########## The level1 simple APIs without input parameters ###########
 ...
 ########## The level2 APIs with input parameters ###########
    ...
    def obstacle_climb_front(self, step_x = 0.045, lift_ht = 0.05, time_uni = 2, time_acc = 2):
        """Manual climb movement for front legs
        Args:
            step_x: horizontal distance of step forward to climb the obstacle (unit: m)
            lift_ht: vertical height of leg lifting (unit: m)
            time_acc: how long it takes to finish the climbing movement (unit: second)
            time_uni: how long pupper will keep still after climbing (unit: second)
        """
        print(f"Inside obstacle_climb_front")
        if time_uni <= 0:
            time_uni = self.dt
        if time_acc <= 0:
            time_acc = self.dt
        ...
        interval_uni = int(time_uni / self.dt)
        interval_acc = int(time_acc / self.dt)
        print(f"Interval uni: {interval_uni}, Interval acc: {interval_acc}")
        ...
        dance_scheme = Movements('climb_front')

        # Base stance (same as default_stand, expanded per leg)
        fr_neutral = np.array([ 0.06,-0.05,-0.07])  # front right
        fl_neutral = np.array([ 0.06, 0.05,-0.07])  # front left
        rr_neutral = np.array([-0.06,-0.05,-0.07])  # rear right
        rl_neutral = np.array([-0.06, 0.05,-0.07])  # rear left
        ...
        rr_p0 = rr_neutral
        rl_p0 = rl_neutral
        #Frame 1: lift rear legs, push fron legs downwards, move front legs backwards
        fr_p1 = fr_neutral + np.array([-step_x_mod, 0, -lift_ht_mod/2.0])
        fl_p1 = fl_neutral + np.array([-step_x_mod, 0, -lift_ht_mod/2.0])
        rr_p1 = rr_neutral + np.array([0, 0, 0])  ### temporary adjustment
        rl_p1 = rl_neutral + np.array([0, 0, lift_ht_mod])


        fr_p2 = fr_p1 + np.array([step_x_mod, 0, lift_ht_mod])
        fl_p2 = fl_p1 + np.array([0, 0, 0])
        rr_p2 = rr_p1 + np.array([0, 0, lift_ht_mod*0.7])
        rl_p2 = rl_p1 + np.array([0, 0, 0])


        fr_p3 = fr_p2 + np.array([step_x_mod * 1.3, 0, -lift_ht_mod/2.0])
        fl_p3 = fl_p2 + np.array([0, 0, 0])
        rr_p3 = rr_p2 + np.array([0, 0, 0])
        rl_p3 = rl_p2 + np.array([0, 0, 0])

        fr_p4 = fr_p3 + np.array([0, 0, 0])
        fl_p4 = fl_p3 + np.array([step_x_mod, 0, lift_ht_mod])
        rr_p4 = rr_p3 + np.array([0, 0, 0])
        rl_p4 = rl_p3 + np.array([0, 0, -lift_ht_mod])

        fr_p5 = fr_p4 + np.array([0, 0, 0])
        fl_p5 = fl_p4 + np.array([step_x_mod * 1.3, 0, -lift_ht_mod/2.0])
        rr_p5 = rr_p4 + np.array([0, 0, 0])
        rl_p5 = rl_p4 + np.array([0, 0, 0])

        # Assemble per-leg keyframes as numpy array: shape (4 legs, phases, 3 coords)
        dance_all_legs = np.array([
            [fr_p0, fr_p1, fr_p2, fr_p3, fr_p4, fr_p5, fr_p0],  # leg 1 front right
            [fl_p0, fl_p1, fl_p2, fl_p3, fl_p4, fl_p5, fl_p0],  # leg 2 front left
            [rr_p0, rr_p1, rr_p2, rr_p3, rr_p4, rr_p5, rr_p0],  # leg 3 rear right
            [rl_p0, rl_p1, rl_p2, rl_p3, rl_p4, rl_p5, rl_p0],  # leg 4 rear left
        ])
        ...
        dance_scheme.setTransitionTic(interval_acc)
        dance_scheme.setInterpolationNumber(interval_uni)
        dance_scheme.setAllSequence(dance_all_legs.tolist(), dance_speed, dance_attitude)
        self.MovementLib.append(dance_scheme)
        return self.MovementLib

    def obstacle_climb_rear(self, step_x = 0.045, lift_ht = 0.05, time_uni = 2, time_acc = 2):
        """Manual climb movement for rear legs
        Args:
            step_x: horizontal distance of step forward to climb the obstacle (unit: m)
            lift_ht: vertical height of leg lifting (unit: m)
            time_acc: how long it takes to finish the climbing movement (unit: second)
            time_uni: how long pupper will keep still after climbing (unit: second)
        """
        print(f"Inside obstacle_climb_rear")
        if time_uni <= 0:
            time_uni = self.dt
        if time_acc <= 0:
            time_acc = self.dt
        ...
        dance_scheme = Movements('climb_rear')
        # Default stance 
        fr_neutral = np.array([ 0.06,-0.05,-0.07])  # front right
        fl_neutral = np.array([ 0.06, 0.05,-0.07])  # front left
        rr_neutral = np.array([-0.06,-0.05,-0.07])  # rear right
        ...
        fr_p1 = fr_neutral + np.array([0, 0, lift_ht_mod*0.75])
        fl_p1 = fl_neutral + np.array([0, 0, lift_ht_mod*0.75])
        rr_p1 = rr_neutral + np.array([-step_x_mod/2, 0, -lift_ht_mod/2.0])
        rl_p1 = rl_neutral + np.array([-step_x_mod/2, 0, -lift_ht_mod/2.0])

        fr_p2 = fr_p1 + np.array([0, 0, 0])
        fl_p2 = fl_p1 + np.array([0, 0, 0])
        rr_p2 = rr_p1 + np.array([0, 0, lift_ht_mod*1.2])
        rl_p2 = rl_p1 + np.array([0, 0, 0])


        fr_p3 = fr_p2 + np.array([0, 0, 0])
        fl_p3 = fl_p2 + np.array([0, 0, 0])
        rr_p3 = rr_p2 + np.array([step_x_mod * 2, 0, -lift_ht_mod/2.0])
        rl_p3 = rl_p2 + np.array([0, 0, 0])

        fr_p4 = fr_p3 + np.array([0, 0, 0])
        fl_p4 = fl_p3 + np.array([0, 0, 0])
        rr_p4 = rr_p3 + np.array([-step_x_mod, 0, -lift_ht_mod/2.0])
        rl_p4 = rl_p3 + np.array([0, 0, lift_ht_mod*1.2])

        fr_p5 = fr_p4 + np.array([0, 0, 0])
        fl_p5 = fl_p4 + np.array([0, 0, 0])
        rr_p5 = rr_p4 + np.array([0, 0, 0])
        rl_p5 = rl_p4 + np.array([step_x_mod * 2, 0, -lift_ht_mod/2.0])
        ...
        dance_speed = []
        dance_speed = [[0, 0, 0] for _ in range(n_frame)]
        dance_attitude = [[0, 0, 0] for _ in range(n_frame)]
        dance_scheme.setTransitionTic(interval_acc)
        dance_scheme.setInterpolationNumber(interval_uni)
        dance_scheme.setAllSequence(dance_all_legs.tolist(), dance_speed, dance_attitude)
        self.MovementLib.append(dance_scheme)
